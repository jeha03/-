...
<!-- Content dasboard BODY -->
<div class="py-12">
  <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
      <div class="p-6 text-gray-900">
        <h2 class="text-3xl font-bold tracking-tight text-gray-900 sm:text-4xl">{{ __('messages.game_acc_dash') }} {{ $username }}</h2>
        <p class="mt-3 text-lg leading-8 text-gray-600">{{ __('messages.number_activate_acc') }} {{ $allow_count }}</p>
      </div>
      <div class="mx-auto mt-10">
        <table class="w-full text-sm text-left text-gray-200 bg-gray-800 dark:text-gray-400">
          <thead>
            <tr>
              <th scope="col" class="px-6 py-4 text-white"><span>№</span></th>
              <th scope="col" class="px-6 py-4 text-white"><span>{{ __('messages.lk_table_dashboard_account') }}</span></th>
              <th scope="col" class="px-6 py-4 text-white"><span>{{ __('messages.lk_table_dashboard_last_data') }}</span></th>
              <th scope="col" class="px-6 py-4 text-white"><span>{{ __('messages.lk_table_dashboard_count_chars') }}</span></th>
              <th scope="col" class="px-6 py-4 text-white"><span>{{ __('messages.lk_table_dashboard_name_server') }}</span></th>
            </tr>
          </thead>
          <tbody>
            @if(isset($arrayInfoDashboard))
              @foreach($arrayInfoDashboard as $model)
                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 text-gray-500">
                  <td data-label="№" class="py-4 text-black">{{ $model->id }}</td>
                  <td data-label="{{ __('messages.lk_table_dashboard_account') }}" class="py-4 text-black">{{ $model->username }}</td>
                  <td data-label="{{ __('messages.lk_table_dashboard_last_data') }}" class="py-4 text-black">{{ $model->dateauth }}</td>
                  <td data-label="{{ __('messages.lk_table_dashboard_count_chars') }}" class="py-4 text-black">{{ $model->count_characters }}</td>
                  <td data-label="{{ __('messages.lk_table_dashboard_name_server') }}" class="py-4 text-black">{{ $model->name_server }}</td>
                </tr>
              @endforeach
            @else
              <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                <td colspan="5" class="px-6 py-4 text-center font-medium text-gray-900 whitespace-nowrap dark:text-white">{{ __('messages.lk_no_data') }}</td>
              </tr>
            @endif
          </tbody>
        </table>
        <div class="buttondashboard">
                  <x-nav-link id="open" onClick="clickOpenDialog({{ $model->server_id }}, '{{ $model->username }}')" class="button19">
                  {{ __('messages.lk_change_password_accounts_title') }}
                              </x-nav-link>
                    <!--<button id="open" onClick="clickOpenDialog({{ $model->server_id }}, '{{ $model->username }}')" class="px-3 py-2 text-xs font-medium text-center text-white bg-blue-700 rounded-lg hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
                      {{ __('messages.lk_change_password_accounts_title') }}
                    </button>-->

                    <x-nav-link id="open_new_account" class="button19">
                    {{ __('messages.lk_add_new_account') }}
                              </x-nav-link>
                      <!--<button id="open_new_account" class="text-white bg-[#FF9119] hover:bg-[#FF9119]/80 focus:ring-4 focus:outline-none focus:ring-[#FF9119]/50 font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center dark:hover:bg-[#FF9119]/80 dark:focus:ring-[#FF9119]/40 mr-2 mb-2">
                        {{ __('messages.lk_add_new_account') }}
                      </button>-->
